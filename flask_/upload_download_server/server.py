...
ALLOWED_EXTENSIONS = set(['txt','jpg','pdf','apk','ppt','doc','png','img','tgz','zip','tar','bz2','wav','xml'])
...
def list_directory(path):
        ...
        f = StringIO()
        displaypath = cgi.escape(urllib.request.unquote(path))
        ...
        for name in list:
            ...
            f.write('<li><a href="uploads/%s">%s</a>\n'
                    % (urllib.request.quote(linkname), cgi.escape(displayname)))
        f.write("</ul>\n<hr>\n</body>\n</html>\n")
        length = f.tell()
        f.seek(0)
