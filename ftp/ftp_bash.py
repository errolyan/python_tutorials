...
class FTP(object):
    ...
    '''
    status = False # 类属性
    def ___init__(self,server,account,password):
        try:
    ...
    def upload(self,file_name):
        if os.path.exists(file_name):
            if os.path.isdir(file_name):
                ...
                self.ftp.storbinary('STOR {file_name}',fp)
                fp.close()
                self.set_dir()
                print('upload:[Success] {self.current()}{file_name}')
        else:
    ...
    def download(self,file_name):
        if file_name in self.dir_list:
            try:
                ...
                print('download: [Success] {self.current()}{file_name}')
            except:
                try:
                    os.remove(file_name)
                    os.mkdir(file_name)
                ...
                print('download: [Error] {self.current()}{file_name}')
    ...
    def cd(self, dir_name):
        try:
            ...
            print('cd: [Error] {self.current()}{dir_name}')
    ...
    def mkdir(self, dir_name):
        if not dir_name in self.dir_list:
            ...
            print('mkdir: [Error] {self.current()}{dir_name}')

    def rm(self, file_name):
        if file_name in self.dir_list:
            try:
                self.ftp.delete(file_name)
                print('rm <file>: [Success] {self.current()}{file_name}')
            except:
                self.cd(file_name)
                for i in self.dir_list:
                    self.rm(i)
                ...
                print('rm <dir>: [Success] {self.current()}{file_name}')
            self.set_dir()
        else:
            print('rm: [Error] {self.current()}{file_name}')
